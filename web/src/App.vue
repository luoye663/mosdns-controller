...
function accountAction(key: string | number) { if (key === 'logout') void logout(); else { password.value = { current: '', next: '', confirm: '' }; passwordError.value = ''; showPassword.value = true } }
async function changePassword() { passwordError.value = ''; if (password.value.next !== password.value.confirm) { passwordError.value = '两次输入的新密码不一致'; return }; passwordSaving.value = true; try { await api.changePassword(password.value.current, password.value.next); showPassword.value = false } catch (error) { passwordError.value = error instanceof Error ? error.message : '修改密码失败' } finally { passwordSaving.value = false } }
