...
export interface QueryEvent { id: number; event_id: string; timestamp_unix_ms: number; client_ip: string; device_name: string; protocol: string; qname: string; qtype: number; qclass: number; rcode: number; route: string; route_source: string; upstream_group: string; upstream_tag: string; cache_hit: boolean; snapshot_version: number; access_rule_id: number; route_rule_id: number; subscription_source_id: number; subscription_source_name: string; subscription_categories: string[]; answer_count: number; answer_min_ttl_seconds: number | null; latency_us: number; error_code: string; error_text: string; result_class: QueryResultClass }
...
export interface Settings { cache_enabled: boolean; cache_ttl: number; query_retention_days: number; database_max_size_gib: number; address_family_mode: 'dual_stack' | 'prefer_ipv4' | 'prefer_ipv6' | 'ipv4_only' | 'ipv6_only' }
