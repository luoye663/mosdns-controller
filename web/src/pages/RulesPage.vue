...
import { api, type GeositeStatus, type Rule, type Version } from '@/lib/api'
...
async function save() { loading.value = true; error.value = ''; try { published.value = editing.value ? await api.updateRule(editing.value.id, { ...draft, id: editing.value.id, updated_at_ms: editing.value.updated_at_ms }) : await api.createRule(draft); showEditor.value = false; await load() } catch (e) { error.value = e instanceof Error ? e.message : '发布失败，已保留编辑内容' } finally { loading.value = false } }
...
<template>
  <section class="page">
    ...
    <div v-else class="table-wrap"><table><thead><tr><th>ID</th><th>模式</th><th>匹配</th><th>优先级</th><th>来源</th><th>备注</th><th>启用</th><th></th></tr></thead><tbody><tr v-for="rule in visibleRules" :key="rule.id"><td>{{ rule.id }}</td><td class="mono">{{ rule.pattern }}</td><td>{{ rule.match_type }}</td><td>{{ rule.priority }}</td><td>{{ rule.source }}</td><td>{{ rule.comment || '-' }}</td><td><NSwitch :value="rule.enabled" @update:value="switchRule(rule, $event)" /></td><td class="actions"><NButton size="small" @click="openEdit(rule)">编辑</NButton><NButton size="small" type="error" secondary @click="remove(rule)">删除</NButton></td></tr><tr v-if="!loading && !visibleRules.length"><td colspan="8" class="empty-cell">该分类没有规则</td></tr></tbody></table></div>
    <NModal v-model:show="showEditor" preset="card" :title="editing ? '编辑规则' : '新增规则'" class="rule-modal"><div class="form-grid"><label>模式<NInput v-model:value="draft.pattern" placeholder="example.com" /></label><label>匹配类型<NSelect v-model:value="draft.match_type" :options="[{ label: '域名后缀', value: 'domain' }, { label: '精确域名', value: 'full' }, { label: '正则表达式', value: 'regexp' }]" /></label><label>优先级<NInputNumber v-model:value="draft.priority" :min="0" :max="1000" /></label><label>备注<NInput v-model:value="draft.comment" maxlength="500" /></label></div><div class="modal-actions"><NButton @click="showEditor = false">取消</NButton><NButton type="primary" :loading="loading" @click="save">保存并发布</NButton></div></NModal>
  </section>
</template>
