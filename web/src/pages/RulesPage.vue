...
const draft = reactive({ category: 'access', action: 'block', upstream_group_id: '', match_type: 'domain', pattern: '', priority: 100, source: 'manual', comment: '', enabled: true })
...
function resetDraft() { Object.assign(draft, { category: selected.value.category, action: selected.value.category === 'route' ? 'upstream' : selected.value.action, upstream_group_id: selected.value.category === 'route' ? preferredGroupID() : '', match_type: 'domain', pattern: '', priority: 100, source: 'manual', comment: '', enabled: true }); editing.value = null }
function openCreate() { resetDraft(); showEditor.value = true }
function openEdit(rule: Rule) { resetDraft(); editing.value = rule; Object.assign(draft, rule); showEditor.value = true }
...
async function save() {
  ...
  try {
    ...
    if (editing.value) {
      ...
      const published = await api.updateRule(editing.value.id, { ...draft, pattern: patterns[0]!, updated_at_ms: editing.value.updated_at_ms, ...registryPrecondition })
      notify.success(`已发布版本 ${published.version}`)
    } else {
      const entries: RuleInput[] = patterns.map((pattern) => ({ ...draft, pattern, ...registryPrecondition }))
...
<template>
  <section class="page">
    ...
    <section v-if="!supportsSubscriptions || ruleView === 'manual'" class="manual-rules">
      ...
      <div class="table-wrap"><table><thead><tr><th>ID</th><th>域名</th><th v-if="selected.category === 'route'">上游组</th><th>匹配</th><th>优先级</th><th>备注</th><th>启用</th><th></th></tr></thead><tbody><tr v-for="rule in visibleRules" :key="rule.id"><td>{{ rule.id }}</td><td class="mono">{{ rule.pattern }}</td><td v-if="selected.category === 'route'">{{ groupName(rule.upstream_group_id ?? '') }}</td><td>{{ rule.match_type }}</td><td>{{ rule.priority }}</td><td>{{ rule.comment || '-' }}</td><td><NSwitch :value="rule.enabled" @update:value="switchRule(rule, $event)" /></td><td class="actions"><NButton size="small" @click="openEdit(rule)">编辑</NButton><NButton size="small" type="error" secondary @click="remove(rule)">删除</NButton></td></tr><tr v-if="!loading && !visibleRules.length"><td :colspan="selected.category === 'route' ? 8 : 7" class="empty-cell">暂无手工规则</td></tr></tbody></table></div>
    </section>
