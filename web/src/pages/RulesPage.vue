...
function resetDraft() { Object.assign(draft, { category: selected.value.category, action: selected.value.category === 'route' ? 'local' : selected.value.action, match_type: 'domain', pattern: '', priority: 100, source: 'manual', comment: '', enabled: true }); editing.value = null }
...
function openSources() { const initialGroup = groups.value.some((group) => group.enabled && group.id === defaultGroupID.value) ? defaultGroupID.value : enabledGroupOptions.value[0]?.value ?? ''; Object.assign(sourceDraft, { url: '', refresh_minutes: 1440, upstream_group_id: initialGroup, priority: 100 }); uploadFiles.value = []; showSourceEditor.value = true }
function openBinding(source: RuleSubscription) { bindingTarget.value = source; Object.assign(bindingDraft, { upstream_group_id: source.binding?.upstream_group_id ?? '', priority: source.binding?.priority ?? 100 }); showBindingEditor.value = true }
...
async function save() {
  ...
  } catch (e) { notify.error(e instanceof Error ? e.message : '发布失败，已保留编辑内容') } finally { loading.value = false }
}
...
async function createSource() {
  ...
  try {
    ...
    const base: Omit<RuleSubscriptionInput, 'source_url'> = { category: selected.value.category, action: selected.value.category === 'route' ? 'local' : selected.value.action, name: '', refresh_interval_seconds: sourceDraft.refresh_minutes * 60, enabled: true, ...routeBinding }
...
async function removeSource(source: RuleSubscription) { if (!window.confirm(`确认删除订阅源 ${source.name}？`)) return; loading.value = true; try { const version = await api.deleteRuleSubscription(source.id); notify.success(`已发布版本 ${version.version}`); await load() } catch (e) { notify.error(e instanceof Error ? e.message : '删除订阅失败') } finally { loading.value = false } }
onMounted(load)
</script>
