...
function newGroup(): UpstreamGroup {
  return { id: '', name: '', enabled: true, mode: 'race', concurrent: 1, max_in_flight: null, query_timeout_ms: null, socks5: '', ecs: { mode: 'off', mask4: 24, mask6: 48 }, cache: { enabled: true, size: 1024 }, upstreams: [{ tag: 'primary', addr: '', priority: 100, weight: 1, timeout_ms: 1000 }] }
}
  ...
  <section class="page">
    ...
    <div class="upstream-group-list">
      <NCard v-for="group in registry?.groups ?? []" :key="group.id" size="small" class="upstream-group-card">
        ...
        <div class="group-settings-grid">
          ...
          <NFormItem label="调度模式"><NSelect v-model:value="group.mode" :options="modeOptions" /></NFormItem>
          ...
          <NFormItem label="组缓存"><NSwitch v-model:value="group.cache.enabled" /></NFormItem>
          <NFormItem label="缓存容量"><NInputNumber v-model:value="group.cache.size" :min="1" :max="65536" /></NFormItem>
          <NFormItem label="ECS"><NSelect v-model:value="group.ecs.mode" :options="ecsOptions" /></NFormItem>
    ...
    <NModal v-model:show="showCreate" preset="card" title="新增上游组" class="upstream-create-modal">
      <div class="group-settings-grid">
        ...
        <NFormItem label="调度模式"><NSelect v-model:value="draft.mode" :options="modeOptions" /></NFormItem>
        ...
        <NFormItem label="缓存容量"><NInputNumber v-model:value="draft.cache.size" :min="1" :max="65536" /></NFormItem>
      </div>
