...
function newGroup(): UpstreamGroup {
  return { id: '', name: '', enabled: true, mode: 'race', concurrent: 1, socks5: '', ecs: { mode: 'off', mask4: 24, mask6: 48 }, cache: { enabled: true, size: 1024 }, upstreams: [{ tag: 'primary', addr: 'https://', priority: 100, weight: 1 }] }
}
...
function addNode(group: UpstreamGroup) { group.upstreams.push({ tag: `upstream_${group.upstreams.length + 1}`, addr: 'https://', priority: 100, weight: 1 }) }
...
<template>
  <section class="page">
    ...
    <NAlert type="info" class="form-alert">默认组在设置页选择。内置 local_dns 与 remote_dns 保留给现有动态规则和订阅路由，其他组可供默认路由及后续订阅绑定使用。</NAlert>
    <div class="upstream-group-list">
      <NCard v-for="group in registry?.groups ?? []" :key="group.id" size="small" class="upstream-group-card">
        ...
        <div class="table-wrap upstream-table-wrap"><table><thead><tr><th>标签</th><th>DNS 地址</th><th>优先级</th><th>权重</th><th></th></tr></thead><tbody><tr v-for="(node, index) in group.upstreams" :key="index"><td><NInput v-model:value="node.tag" /></td><td><NInput v-model:value="node.addr" placeholder="https://dns.example/dns-query" /></td><td><NInputNumber v-model:value="node.priority" :min="1" :max="1000" /></td><td><NInputNumber v-model:value="node.weight" :min="1" :max="100" /></td><td><NTooltip><template #trigger><NButton type="error" quaternary :disabled="group.upstreams.length === 1" aria-label="移除节点" @click="removeNode(group, index)"><template #icon><Trash2 /></template></NButton></template>移除节点</NTooltip></td></tr></tbody></table></div>
