...
async function rollback(version: Version) { if (!window.confirm(`确认将版本 ${version.version} 的规则重新发布为新版本？`)) return; loading.value = true; try { const result = await api.rollback(version.version, upstreamRegistryVersion.value); notify.success(`已创建并发布版本 ${result.version}`); await load() } catch (e) { if ((e as ApiError).status === 409) await load(); notify.error(e instanceof Error ? e.message : '回滚失败') } finally { loading.value = false } }
