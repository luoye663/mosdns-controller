...
async function load() { loading.value = true; try { const [nextSettings, nextGeosite] = await Promise.all([api.settings(), api.geositeStatus()]); settings.value = nextSettings; geosite.value = nextGeosite; geositeSource.value = nextGeosite.source_url } catch (e) { notify.error(e instanceof Error ? e.message : '无法读取设置') } finally { loading.value = false } }
...
async function updateGeosite() { saving.value = true; try { geosite.value = await api.updateGeosite(geositeSource.value); geositeSource.value = geosite.value.source_url; notify.success(`国内域名集合已更新，版本 ${geosite.value.version}`) } catch (e) { notify.error(e instanceof Error ? e.message : '更新国内域名集合失败') } finally { saving.value = false } }
...
async function uploadGeosite() { if (!geositeFile.value) return; saving.value = true; try { geosite.value = await api.uploadGeosite(geositeFile.value); geositeSource.value = ''; geositeFile.value = null; if (uploadInput.value) uploadInput.value.value = ''; notify.success(`国内域名集合已上传，版本 ${geosite.value.version}`) } catch (e) { notify.error(e instanceof Error ? e.message : '上传国内域名集合失败') } finally { saving.value = false } }
onMounted(load)
</script>
